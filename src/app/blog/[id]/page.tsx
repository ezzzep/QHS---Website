...
export default function BlogDetail() {
  ...
  // Get the current URL after component mounts
  useEffect(() => {
    if (typeof window !== "undefined") {
      // For development, check if we're using ngrok or similar
      const hostname = window.location.hostname;
      if (hostname === "localhost") {
        console.warn(
          "Facebook sharing will not work with localhost. Use ngrok or deploy to a public URL."
        );
      }
  ...
  // Create a description for Open Graph (first 150 characters of content)
  const ogDescription =
    blog.content.length > 150
      ? blog.content.substring(0, 150) + "..."
      : blog.content;

  // Get the image URL for Open Graph, fallback to a default image if none exists
  // Make sure the image URL is absolute and publicly accessible
  const getAbsoluteImageUrl = (url: string | null) => {
    if (!url) return "https://your-domain.com/default-og-image.jpg";
  ...
  const ogImage = getAbsoluteImageUrl(blog.cover_image_url);

  // Get the absolute URL for the current page
  const getAbsoluteUrl = () => {
    if (currentUrl && currentUrl.startsWith("http")) return currentUrl;

    // For development, use ngrok URL if available, otherwise fallback to localhost
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || window.location.origin;
    ...
    <>
      ...
      <Head>
        ...
        <meta
          ...
          content={blog.author_name || "Anonymous"}
        />
        <meta
          property="article:published_time"
          content={new Date(blog.created_at).toISOString()}
        />

        {/* Twitter Card tags (optional but good for Twitter sharing) */}
        ...
        {/* Facebook Debug Tool Warning */}
        {process.env.NODE_ENV === "development" && (
          <script
            dangerouslySetInnerHTML={{
              __html: `
                console.warn('Facebook sharing will not work with localhost. Use ngrok or deploy to a public URL.');
                console.log('Current URL for sharing:', '${absoluteUrl}');
          ...
          <div className="max-w-4xl mx-auto px-6 py-4">
            {process.env.NODE_ENV === "development" && (
              <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-2 rounded mb-2">
                <p className="text-sm">
                  <strong>Development Mode:</strong> Facebook sharing won't work
                  with localhost. Use ngrok or deploy to a public URL to test
                  sharing.
                </p>
                <p className="text-xs mt-1">Current URL: {absoluteUrl}</p>
              </div>
            )}
          </div>
        </div>
